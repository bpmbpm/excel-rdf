...
(function (global) {
  ...
  function tokenize(text) {
    ...
    while (i < n) {
      ...
      // Фигурные скобки { } — структурные символы именованных графов TriG (см. js/trig.js).
  ...
  /*
   * Разбить значение ячейки на отдельные объекты (термы), разделённые запятой.
   * Запятые ВНУТРИ литералов «"..."» / «'...'» и IRI «<...>» не считаются разделителями,
   * поэтому литерал вида «"Второй процесс, выполняется после"» остаётся целым.
   * Возвращает массив непустых термов (с обрезанными пробелами).
   */
  function splitObjects(value) {
    ...
    for (var i = 0; i < s.length; i++) {
      ...
      if (inIri) { buf += ch; if (ch === '>') { inIri = false; } continue; }
      if (ch === '"' || ch === "'") { inString = true; stringChar = ch; buf += ch; continue; }
      if (ch === '<') { inIri = true; buf += ch; continue; }
