 ...
 *   serializeTrigSimple(model) -> строка TriG (простая форма: по одному триплету на строку)
 *
...
(function (global) {
  ...
  /*
   * Собрать TriG-текст в ПРОСТОЙ форме: по одному элементарному триплету на строку
   * (без ; и ,), но с сохранением блоков именованных графов «g { ... }».
   * Это аналог serializeTurtleSimple для TriG — «внутри TriG только простые триплеты».
   */
  function serializeTrigSimple(model) {
    var prefixes = model.prefixes || {};
    var quads = model.quads || [];
    var lines = [];
    ...
    // Группировка по графу с сохранением порядка появления.
    var graphOrder = [], byGraph = {};
    quads.forEach(function (q) {
      var key = q.g == null ? ' default' : q.g;
      if (!byGraph[key]) { byGraph[key] = { term: q.g, triples: [] }; graphOrder.push(key); }
    ...
    graphOrder.forEach(function (key) {
      var grp = byGraph[key];
      if (grp.term == null) {
        // Граф по умолчанию — без скобок, по одному триплету на строку.
        grp.triples.forEach(function (t) {
  ...
  var api = {
    ...
    serializeTrigSimple: serializeTrigSimple
  };
