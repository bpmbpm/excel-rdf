/*
 * workbook.js — преобразование модели Turtle в книгу Excel и обратно (редакция ver2).
 *
 ...
 *
 * Отличия ver2 от ver1:
 *   • На листе main для листов МегаТипов в отдельном столбце «Предикаты» перечислены все
 *     предикаты (поля) данного листа — чтобы наборы предикатов разных МегаТипов можно
 *     было сравнивать в одном месте (требование 3).
 *   • Книга формируется через js/xlsx-extras.js: на каждом листе закрепляется первая
 *     строка и включается автофильтр (требование 1).
 *
 * Состав книги (концепция проекта, см. doc/):
 *   1. main                  — перечень листов книги, их назначение и предикаты листов МегаТипов
...
(function (global) {
  ...
  var PRED_SEP = ' , ';  // разделитель предикатов в столбце «Предикаты» листа main
  ...
  function modelToSheets(model) {
    ...
    // Третий столбец «Предикаты» — для листов МегаТипов перечисляет их поля (требование 3).
    ...
    // Заполняем main описанием листов и (для МегаТипов) перечнем предикатов.
    var descriptions = {};
    ...
    var predsByName = {}; // имя листа -> строка с перечнем предикатов
    megaSheetInfo.forEach(function (info) {
  ...
  /*
   * Собрать готовый .xlsx (Uint8Array) из модели Turtle: с закреплением первой строки
   * и автофильтром на каждом листе (через xlsx-extras.js).
   */
  ...
  function sheetsToModel(book) {
    ...
    function pushCell(subject, predicate, cellVal) {
      // Несколько объектов в одной ячейке разделяются запятой; запятые внутри
      // литералов «"..."» и IRI «<...>» разделителями не считаются (T.splitObjects).
      T.splitObjects(cellVal).forEach(function (v) {
        triples.push({ s: subject, p: predicate, o: v });
      });
    }
